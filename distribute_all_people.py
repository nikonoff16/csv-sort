...
def distribute_all_people(name, lst_csv, by_months=True):
    """
    Функция переводит подготовленные двухуровневые списки в csv-формат. Она умеет находить как список всех подлежащих
    диспансеризации, так и делать разбивку этого списка по месяцам, для составления плана работы на год.
    :param name: строка, которая будет именем нового файла
    :param lst_csv: двухуровневый список из форматированных под cvs строк
    :param by_months: если True - вывести список подлежащих исследованию, распределенный на группы по месяцам.
